...
centroid = (2 * b1 * d1_b + 2 * b2 * d2_b + 2 * b3 * d3_b + 2 * d4_b) / (2 * d1 + 2 * d2 + 2 * d3 + 2 * b4)
...
qs0 = (
    - q12*(-x2+x1)*y1 + q12*(y2-y1)*x1
    - q23*(-x2+x2)*y2 + q23*(y3-y2)*x2
    - q34*(-x1+x2)*y3 + q34*(y4-y3)*x2
    - q45*(x1+x1)*y4 + q45*(y4-y4)*x1
    - q56*(x2-x1)*y4 + q56*(y3-y4)*x1
    - q67*(x2-x2)*y3 + q67*(y2-y3)*x2
    - q78*(x1-x2)*y2 + q78*(y1-y2)*x2
    - q18*(x1+x1)*y1 + q18*(y1-y1)*x1
) / (2*A)
